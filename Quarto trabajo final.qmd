...
Además de clasificar a los jugadores como All Star o No All Star, el modelo también permite estimar las probabilidades de que un jugador sea seleccionado al All Star. A continuación se desarrollara una aplicación Shiny aprovechando esta característica.
...
A raíz de lo presentado podemos concluir que el modelo predice con bajo margen de error si un jugador llega a ser All-Star o no.
...
prueba <- datos %>% 
  filter(G >= 60) %>% 
  select(Player.x, Pos1, G, MP, TRB, AST, STL, BLK, PTS, Play, Season)
```

robos:

```{r}
ggplot(prueba, aes(x = Play, y = PTS, fill = Play)) +
  geom_boxplot(alpha = 0.8, outlier.shape = NA) +
  facet_wrap(~Season) +
  geom_jitter(width = 0.15, alpha = 0.5, size = 1.5) +
  scale_x_discrete(labels = c("No" = "No All-Star", "Yes" = "All-Star")) +
  labs(
    title = "Puntos por partido según condición de All-Star por temporada", 
    x = "", y = "Puntos por partido") +
  theme_classic() +
  theme(legend.position = "no")
```
